...
**Making your own skill.** `/makeskill` turns what a chat just worked out into a reusable skill: the model writes it as a `SKILL.md`, the draft is checked before anything is kept (a name, a description that says *when* to use it, a body worth following), and it is saved and pinned to that chat. Yours lead the picker, so a name of yours wins over a library one, and typing `/<name>` in any chat uses it again. They live in this browser beside the conversations — up to 30, 20 kB each — because the deployment is shared and its container is rebuilt on every push.
...
In Chat and Plan it lives in your browser (localStorage) beside the conversations, not on the server. The deployment is shared and its container is rebuilt on every push, so a server-side workspace would be both visible to other people and temporary. Files can be downloaded or deleted from **Settings → Workspace files**.

**In Build mode, on a server with `WORKSPACE_RUN=1`, the same six tools act on the server folder that `run_command` runs in.** That is what makes a coding task possible: the file the model writes is the file its command runs, and `git` can commit it. Clone a repository with `run_command` (`git clone …`), then search, read and edit it with the workspace tools; the files appear under **Settings → Server files**. Git runs as the GitHub account you connected — the token is handed to git in its environment, never shown, and scrubbed from any output — and force pushes, `--amend`, `rebase` and `-i` are refused before you are asked. A model that writes its tool call as text (`<tool_call>`, `<function=…>`, `[TOOL_CALLS]` or a JSON fence) is understood too: the call runs and the markup never reaches the transcript. Build mode also gets 60 tool steps a turn where Chat keeps 12.
