...
| `CUSTOM2_BASE_URL` | *(unset)* | A second generic gateway slot, for running two self-hosted gateways at once — e.g. a [CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI) wrapping your free agent-CLI logins (see `deploy/cliproxy-railway/`). Behaves exactly like `CUSTOM_BASE_URL`: activates on the URL alone, key optional via `CUSTOM2_API_KEY`, `CUSTOM2_MODELS` pins the picker. |
| `CUSTOM3_BASE_URL` | *(unset)* | A third generic gateway slot — e.g. a [Kiro Gateway](https://github.com/jwadow/kiro-gateway) serving free-tier Claude Sonnet 4.5 (see `deploy/kiro-gateway-railway/`). Behaves exactly like `CUSTOM_BASE_URL`: activates on the URL alone, key optional via `CUSTOM3_API_KEY`, `CUSTOM3_MODELS` pins the picker. |
