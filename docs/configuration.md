...
| `FREEBUFF_BASE_URL` | *(unset)* | Points the **Freebuff** provider at a self-hosted Freebuff2API proxy (see `deploy/freebuff-railway/`) - an OpenAI-compatible front for Freebuff free coding models. Include `/v1` or leave it off - the version segment is added when it is missing. The provider appears once this is set; the model list is read live from the proxy, `FREEBUFF_API_KEY` is only sent when set (matching a proxy deployed with `API_KEYS`), `FREEBUFF_MODELS` pins a subset and `FREEBUFF_DISABLED=1` switches it off. Chat only: the proxy serves no image endpoint. |
