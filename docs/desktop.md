...
The status bar carries one **update button** beside the version, so an update never depends on finding the command palette: *Check for updates* → *Checking…* → *Up to date* (briefly), *Update to vX.Y.Z* (the same download, verify and install as the banner) or *Couldn't check* (the reason on hover). A check someone asks for reports a version they dismissed earlier; the hourly poll still respects the dismissal. The shell's `install_kind` says how this copy was installed — an `uninstall.exe` beside the exe is the NSIS setup, an exe under Program Files without one is the MSI, anything else is portable — and the update uses the same kind, because the other installer would register a second copy instead of upgrading; a portable copy only downloads and verifies the new exe and says where it was saved. The repo stays at `x.y.0` and CI stamps the patch with the workflow's run number before building, so every build is strictly newer than the last and Windows Installer will actually upgrade an MSI.
...
### Mockups from viralai (NEURA-070)

**What transfers, plainly: the slide compositor, and nothing else.** The `viralai` repo is a social-post pipeline (FastAPI, Celery, platform copy prompts, Telegram/Slack approvals); its mockup generator is one function — `generate_carousel_image` — that draws a 1080×1080 card: dark paper, a 12px accent bar whose hue comes from the copy itself, measured word-wrap, white 56px bold centred on a 70px rhythm. That compositor is now [`design/mockups.js`](../desktop/src/design/mockups.js), planned as pure numbers and painted on a canvas. Three things changed, each on purpose: the accent hue comes from FNV-1a instead of Python's per-process-salted `hash()` (the same text makes the same card on every run); the metrics scale with width/1080 so the 1080×1350 and 1200×675 formats keep the composition; and the never-drawn `font_small` did not come along. Copy is capped and overflow elides with an ellipsis, so a long model answer cannot run off the card.

**Mockups tab.** The Design inspector gains a **Mockups** tab. **Generate from canvas** takes each slide's own copy from the page (or add slides and type them); edit and the card re-renders at full size — the preview is exactly what exports. Card sizes are the social templates' own platform formats. **Export PNG** saves the slide under edit through the normal save dialog; **Export all (ZIP)** packs the set. The page is never rewritten from a mockup: this is the raster side of the studio.
