...
| NEURA-072 | desktop | upgrade | M | **Add from Hugging Face for Images and Dictation**: the in-app downloader is GGUF-only because it feeds llama-server, so sd-server weights (`.safetensors`, `.gguf`) and whisper models (`.bin`) are fetched by hand. Each card gets its own downloader with its own formats and folder: single-file checkpoints, GGUF, and multi-file component sets (diffusion model + VAE + text encoders, fetched as one unit); a diffusers-layout repo yields its merged root file when one exists, and says plainly when it has nothing sd-server can load. | desktop |
| NEURA-074 | desktop | fix | S | **Cancel means what this sd-server can do**: the pinned stable-diffusion.cpp build reports `cancel_generating: false` in `/sdcpp/v1/capabilities` -- a queued job cancels, one already drawing runs to the end. Read the capability and label the button honestly ("Stop after this picture" while drawing) instead of offering a cancel that does nothing. | desktop |
...
| ID | Commit | What |
|---|---|---|
| NEURA-077 | `3a46597` | Inpainting on this PC: a brush over the picture in Images > Change a picture (paint, erase, size, clear) draws a black-and-white mask at the picture's own size and sends it as sd.cpp's `mask_image` beside `init_image`. Measured on a GTX 1050 Ti with sd-v1-5-inpainting: an RGBA PNG mask is accepted, white is repainted and black kept; strength 1.0 redraws the masked area, 0.75 left it unchanged |
| NEURA-076 | `af5b75a` | One update button for both installers: an always-visible status-bar control (Check for updates → Checking… → Up to date / Update to vX.Y.Z / Couldn't check) that reuses the banner's download-verify-install flow and, when asked, reports a version the user had dismissed; the shell's `install_kind` (an `uninstall.exe` beside the exe = NSIS, under Program Files = MSI, else portable) picks the matching artifact, so an MSI install is upgraded by the .msi and a portable copy only saves the new exe; and CI stamps the patch with the run number into package.json, tauri.conf.json, Cargo.toml and version.ts, so every build is newer than the last and Windows Installer will upgrade it |
| NEURA-075 | `105c58c` | An instruction-edit model (a file or set named `edit` / `kontext`: Krea2 edit, Qwen-Image-Edit, Flux Kontext) gets the picture in `ref_images` instead of `init_image` + `strength`; and a multi-file model started on a card whose Vulkan line says `fp16: 0` shows a warning in Images, from sd-server's own log, before anyone waits minutes for a white picture |
| NEURA-073 | `4709da1` | A multi-file image model (Krea2, Flux, Qwen-Image) in its own folder under the image models folder is listed as one "set of N files" and started with each part under its flag: `--diffusion-model`, `--vae`, `--llm`, `--llm_vision` (the mmproj, without which an edit model cannot see its picture), `--clip_l`, `--t5xxl` |
| NEURA-070 | `73bd0ab` | The viralai mockup generator in the Design studio: the slide compositor (accent bar, hue from the copy, measured wrap, centred 56/70 type) ported to `design/mockups.js`, plus a Mockups tab -- generate from the canvas deck or typed slides, edit to re-render at full size, export one PNG or the set as a ZIP. What transfers (and what deliberately changed) is said plainly in `docs/desktop.md`. |
