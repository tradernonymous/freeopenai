...
| NEURA-073 | desktop | upgrade | S | **Start a multi-file image model**: NEURA-072 downloads a Flux / Qwen-Image component set (diffusion model + VAE + text encoders) into `sd-models/<name>/`, but `sd_start` only passes sd-server a single `-m <file>`. A set needs `--diffusion-model`, `--vae`, `--clip_l`, `--t5xxl` / `--llm` by role; the roles are already detected at download time. | desktop |
