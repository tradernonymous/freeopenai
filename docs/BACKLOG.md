...
| NEURA-075 | desktop | fix | M | **An edit model gets the picture as a reference, and a big model gets told the truth about this GPU**: the local edit route sends `init_image` + `strength` (img2img), but an instruction-edit model (Krea2 edit, Qwen-Image-Edit) wants the picture in `ref_images`. Measured on a GTX 1050 Ti (Vulkan reports `fp16: 0 bf16: 0`): Krea2 on Vulkan decodes pure white whatever the cfg, flash attention or VAE placement; on CPU it runs (~230 s a step at 256 px) but the `Krea2_turbo_uncensored_edit_v1.1` Q4_K_M file never denoised in 4 steps. Route edit models through `ref_images`, and before starting a large set on a card with no fp16, say it will be slow or wrong rather than hand back a white square. | desktop |
