...
- **Knowledges:** one hub for everything that teaches a chat something — **Skills** from the server's catalogue (each opens its full `SKILL.md`, and **Use** starts a chat with it pinned), personas, the prompt library, and the image gallery — plus the command list.
- **Commands:** `/help`, `/skill <name>` (or just `/skillname`), `/skills`, `/mode chat | plan`, `/clear`, `/compact on | off`, `/doctor`. They act on the phone — a pinned skill's text is added to that chat's system prompt, `/compact` shortens what is sent without touching what is on screen, and `/doctor` reports providers, models, skills, mode, pinned skills and the server's own budget. `/mode build` says plainly that builds run on the web/desktop app. A line that merely starts with a slash but names nothing installed is still sent as a normal message.
