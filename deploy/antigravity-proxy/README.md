...
| Every account returns `400 invalid argument` (and sometimes `403 Verify your account`) on every endpoint, while the same accounts work from the operator's machine | the pinned 0.7.0 image impersonates Antigravity client 1.15.8, which Google's API now rejects — the archived upstream fixed this in v1.7.1 (client `2.0.1`), and the Dockerfile here applies that header patch at build time. If a redeploy still 400s after this, the remaining suspect is the datacenter IP itself; sign the accounts in again locally and re-paste `AG_ACCOUNTS_JSON` |
| `[Manager] Loaded 0 accounts from storage.` in the proxy's deploy logs | the same fault seen from the proxy's side: `AG_ACCOUNTS_JSON` was empty or unset at boot. The entrypoint now prints a warning naming the fix — paste the contents of the local `antigravity-accounts.json` into `AG_ACCOUNTS_JSON` and redeploy |
