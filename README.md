...
| `OLLAMA_IMAGE_MODEL` | *(read from the tags you have pulled)* | A local image model (a `*-image` tag). Without one, the tags this server has pulled are read for a model that reads as an image model. |
| `<PROVIDER>_IMAGE_MODEL` | *(read from the provider's catalogue)* | **Pins or overrides** the model any other provider draws with — `GEMINI_IMAGE_MODEL`, `ANTIGRAVITY_IMAGE_MODEL`, `MISTRAL_IMAGE_MODEL`, `GROQ_IMAGE_MODEL` and so on, for every provider this app can chat on. Usually unnecessary now: a configured service's own catalogue is read for a model that reads as an image model, so a Google key or a proxy draws without it. Set it to choose a specific model, or on a catalogue this app cannot read. A named provider is tried behind the seven built-ins, and first when the conversation is on it; a failure names this variable only after the catalogue has been read and had nothing. `<PROVIDER>_IMAGES_BASE_URL` overrides the host when its images do not live where its chat does. Speech and search services (Deepgram, AssemblyAI, You.com) are never candidates, whatever is named. |
...
| `CLOUDFLARE_IMAGE_MODEL` | `@cf/black-forest-labs/flux-1-schnell` | Which Workers AI image model to draw with. The default is the cheapest in the catalogue by a wide margin — **4.80 neurons per 512×512 tile**, against 530–1363 for `@cf/leonardo/*` and `@cf/black-forest-labs/flux-2-*` — which is what makes the 10,000 free neurons a day worth hundreds of pictures instead of a handful. `@cf/black-forest-labs/flux-2-klein-4b` (5.37 in / 26.05 out per tile) is the quality upgrade and also does editing; `@cf/runwayml/stable-diffusion-v1-5-inpainting` takes a brush mask. |
| `POLLINATIONS_FREE` | `1` | Whether the **keyless** free drawer may be used: `pollinations.ai` takes no account at all, so a deployment with no provider key of any kind still draws. Set `0` to refuse it — a prompt going to a public community service is your call even when it is free. Anonymous draws are watermarked and capped somewhere under a megapixel with the requested shape preserved. |
| `POLLINATIONS_TOKEN` | *(unset)* | Optional free account key, which asks for the picture without the watermark. |
| `POLLINATIONS_IMAGE_MODEL` | `flux` | Which Pollinations model to draw with. |
| `GEMINI_IMAGE_MODEL` | `gemini-3.1-flash-image` | Which Google image model draws. Current ids are `gemini-3.1-flash-image` (Nano Banana 2, the generalist), `gemini-3.1-flash-lite-image` (fastest and cheapest, but Google documents it as unfit for multi-turn editing), `gemini-3-pro-image` (Nano Banana Pro) and the legacy `gemini-2.5-flash-image`, which shuts down on 2026‑10‑02. |
| `GEMINI_IMAGES_BASE_URL` | *(https://generativelanguage.googleapis.com/v1beta)* | Override for Google's image host. It is the *native* API, not the `/openai` shim the chat uses — that shim has no image endpoint at all. |
...
| `GEMINI_API_KEY` | *(unset)* | Adds **Gemini** through Google's OpenAI compatibility shim at `/v1beta/openai`, not the native API. Free tier, no card. Its catalogue names models `models/gemini-2.5-flash`; the prefix is stripped so the picker and the history hold the id the shim's own docs pass. **Drawing uses the native API**, which is a different host from the chat's: see `GEMINI_IMAGE_MODEL`. |
...
Every provider this app chats on can also draw, because they are all asked the same question through the same route: the server's `/api/llm/images/{generations,edits}`, behind which sits an order — **Nara → Cloudflare → OpenRouter → NVIDIA → HuggingFace → OmniRoute → Ollama**, then any provider you have pointed at a model yourself, then the one that needs no key — with the chat's own service moved to the front of it. There is no second picker to configure: a request for a picture is still a request to whoever is answering the chat.

**Drawing for free, in the order worth trying.** A free key is the whole point of this app, and the four ways to spend one on pictures are not equally good:

1. **Cloudflare Workers AI** — the free tier includes text-to-image, it is a *daily* allowance that refills, and 10,000 neurons goes a long way: at 4.80 neurons per 512×512 tile, `flux-1-schnell` is on the order of a couple of hundred 1024px pictures a day, against a handful from the Leonardo models at 530–636 neurons a tile. Two variables and no card. This is the one to set up.
2. **Puter** — already free, already drawing, and needs nothing at all from you. It spends the *visitor's* monthly credits rather than your key, which is why the app asks before using it (see the Draw-with-Puter switch below).
3. **Pollinations** — genuinely keyless: no signup, no card, no variable. Its endpoint is one GET whose path is the prompt and whose answer is the picture, and it honours the width and height it is handed. **Its anonymous tier watermarks what it makes and caps it under a megapixel**, which is the whole reason it is last in the order and why the picture announces it: every drawing that comes back from here carries the sentence *"drawn on Pollinations’ anonymous tier, which watermark its pictures — a free account token for it, or any other provider in the order, draws without a visible one"* on the status line. A mark you can see is a disclosure; the same mark discovered in a client's presentation is a bug. `POLLINATIONS_FREE=0` refuses the drawer outright. It earns its place twice over: it is the only drawer that reaches the *server* rather than the browser, so a user with no Puter account still gets a picture, and it is what every configured-but-unable-to-draw key now falls back to instead of an error.
4. **Google, via `GEMINI_API_KEY`** — the model is excellent and the free tier is real but *small and moving*: the 2.5 Flash Image generation was free at ~500 requests a day through an AI Studio key, the 3.x generations have been tightened to the point of zero quota on some accounts, and `gemini-2.5-flash-image` is retired on 2026‑10‑02. Treat it as a good key you may already have, not as the plan.

What is *not* on that list, deliberately: **OpenRouter's Image API has no free tier at all** (its docs say so, and no image model there carries a `:free` id), HuggingFace's allowance is monthly credits worth about ten cents, NVIDIA's is signup credit that runs out once, and **no Kimi or Moonshot model generates images** — the Kimi family reads pictures and writes text, so a `kimi-k3` chat is best sent to a service that draws rather than asked to.

**Every provider that is configured can draw, and nobody has to name a model to make that true.** Seven services ship an image model already named; for every other one, the model comes from the provider's *own catalogue* — read once, for the service the request would ask first, and cached for the same twenty minutes the model picker uses. A chat on an Antigravity proxy draws on `gemini-2.5-flash-image` because that id is in the catalogue the proxy already serves, which is the one line of configuration an operator could not otherwise have known to write. (Google's own key is the exception, and it is a declared store rather than a discovered one: its pictures are not on the endpoint its chat uses.) It is read only when the service has no image model of its own, so a provider with a default or a variable costs nothing, and a catalogue that answers "none" is not read again until it expires. `<PROVIDER>_IMAGE_MODEL` still exists and still wins — it is how a model is *pinned* or overridden, rather than the only way to have one — and `<PROVIDER>_IMAGES_BASE_URL` still handles a host that is not the one its chat uses.
...
**When nothing can draw, the sentence leads with the provider the chat is on.** A deployment whose chats run through a proxy has no Nara key and never will, so a list of seven services it does not have is advice it cannot take — and it never once mentioned the provider the user had just been chatting on. The conversation's provider now comes first, with its own variable: `mistral (Mistral has no image model named — set MISTRAL_IMAGE_MODEL.)` — and that is only said after its catalogue has been read and had nothing to offer, so it is the one variable that would actually fix it.
...
| `gemini-image` | `{model, input:[{type:"text"}, {type:"image"}], response_format?}` to `/interactions` | the base64 block inside the interaction |
| `pollinations` | `GET /prompt/<prompt>?width&height&model` | the picture's own bytes |
...
**A picture that carries a mark says so.** A service's own caveat travels on the response's `notes` next to the pictures — `store.caveat`, which may be a sentence or a function of the provider when the answer depends on how it is configured. One service needs it: Pollinations' free tier watermarks, so an unwatermarked picture and a watermarked one are told apart on screen rather than in a client's slide deck.

**Three reasons a draw used to fail on every provider at once.** All three were live at the same time, which is why "generate an image" reported one long sentence naming every service:
...
- **Gemini answered `404` to `/images/generations` on its own OpenAI shim.** Every other chat provider without a declared image store gets one derived from its chat base URL, which is right when "OpenAI-compatible" means the whole surface — and wrong for Google, whose shim carries chat and embeddings and no images at all. So a `GEMINI_API_KEY` could chat for a month and never draw, and the failure read as "no image provider is ready", naming a key the operator had already set. Gemini now declares a `gemini-image` store pointed at the native `/interactions` API, which is where its pictures actually live.
- **HuggingFace answered `410 The requested model is deprecated`.** `FLUX.1-schnell` was retired from `hf-inference`. The default is now `stabilityai/stable-diffusion-3-medium-diffusers`, which the Hub's own provider mapping lists as live. It is **gated**: accept the licence once on [its model page](https://huggingface.co/stabilityai/stable-diffusion-3-medium-diffusers) or a token gets a `403` instead of a picture. `HF_IMAGE_MODEL` overrides it.
