...
| `OMNIROUTE_IMAGE_MODEL` | *(unset)* | Which image model your gateway has connected, as `provider/model` (e.g. `openai/gpt-image-2`). OmniRoute is never a candidate until one is named — it is your gateway's catalogue, and nothing here can know what is loaded there. **Its image endpoint serves a much smaller provider set than its chat endpoint**: OpenAI, xAI, Together, Fireworks, Nebius, Hyperbolic, NanoBanana, OpenRouter, and local SD WebUI / ComfyUI. A chat model id from any other namespace is refused with `400 Invalid image model … Use format: provider/model`, however valid it looks in the chat catalogue — a Cloudflare Workers AI id, for instance, cannot draw here at all. |
| `OLLAMA_IMAGE_MODEL` | *(unset)* | A local image model (a `*-image` tag). Same rule: named or not offered. |
| `<PROVIDER>_IMAGE_MODEL` | *(unset)* | Makes **any other provider** draw with the model named here — `GEMINI_IMAGE_MODEL`, `ANTIGRAVITY_IMAGE_MODEL`, `MISTRAL_IMAGE_MODEL`, `GROQ_IMAGE_MODEL` and so on, for every provider this app can chat on. Naming one is the whole requirement: the request shape is the OpenAI one its chat already speaks. Without it a chat provider is not a candidate — `/api/llm/images/providers` reports it, and a failure names the variable — and with it the provider is tried behind the seven built-ins, and first when the conversation is on it. `<PROVIDER>_IMAGES_BASE_URL` overrides the host when its images do not live where its chat does. Speech and search services (Deepgram, AssemblyAI, You.com) are never candidates, whatever is named. |
...
**Seven providers are wired up with an image model already named, and any other provider joins the order the moment its operator names one.** `<PROVIDER>_IMAGE_MODEL` is the model, `<PROVIDER>_IMAGES_BASE_URL` is a host that is not the one its chat uses, and that is the whole requirement — because everything else about drawing is already the OpenAI shape the chat path speaks. `ANTIGRAVITY_IMAGE_MODEL=gemini-2.5-flash-image` is enough for a chat on the Antigravity proxy to produce pictures, and the same line works for `GEMINI_IMAGE_MODEL`, `MISTRAL_IMAGE_MODEL`, `GROQ_IMAGE_MODEL` and any gateway this app can chat on. The named ones are tried behind the seven, in declaration order.
...
**When nothing can draw, the sentence leads with the provider the chat is on.** A deployment whose chats run through a proxy has no Nara key and never will, so a list of seven services it does not have is advice it cannot take — and it never once mentioned the provider the user had just been chatting on. The conversation's provider now comes first, with its own variable: `mistral (Mistral can draw once its image model is named — set MISTRAL_IMAGE_MODEL.)`.
