...
Every provider this app chats on can also draw, because they are all asked the same question through the same route. **The conversation's own service is asked first, with the conversation's own model**: a request for a picture is still a request to whoever is answering the chat, so there is no second picker to configure and no way to end up with a picture drawn by a service you were not using. When the chat is on Puter — which draws in the *browser* on the visitor's own account, costing the operator nothing and needing no configuration at all — Puter is asked first with that model, and the server's `/api/llm/images/{generations,edits}` is asked when Puter is not signed in, or when it says no. Behind the route is an order: **Nara → OpenRouter → NVIDIA → HuggingFace → OmniRoute → Ollama**, with the chat's service moved to the front of it.
