...
| `NARA_IMAGE_MODEL` | *(e.g. `gpt-image-2.5`)* | Image-capable alias for `/api/llm/images/generations` and `/api/llm/images/edits`. Required — both refuse clearly without it, rather than passing on Nara's "Image model is required". The upstream supports only a fixed set of dimensions; anything else returns 400 with the allowed list (`1024x1024`, `1640x856`, `1024x1280`, `2048x1024`) rather than being rewritten silently. `quality`, `size` and `n` are passed on when a caller asks for them — and dropped, with one retry, if the front refuses the request for their sake. |
