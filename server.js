...
const LLM_PROVIDERS = {
  ...
  // OmniRoute and Freebuff both stay out of the picker until a variable points
  // at them. One variable buys a free drawer with no key at all.
  //
...
function catalogueRows(data) {
  ...
  return rows
    // A catalogue can be a bare string array -- some self-hosted gateways
    // answer /models with ["gpt-4", ...]. Those become bare ids; anything else
...
function buildCapableProvider(id) {
  ...
  if (provider.kind && provider.kind !== 'chat') return null;
  return provider;
}
