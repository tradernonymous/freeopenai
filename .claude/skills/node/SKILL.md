---
name: node
description: "Node.js server practices. Use when writing or debugging server.js and friends: node:test flakiness or hangs, streams and SSE, error handling, graceful shutdown on Railway redeploys, timeouts, performance."
metadata:
  tags: node, nodejs, javascript, typescript, type-stripping, backend, server
---

> **freeopenai notes:** this project is plain JavaScript (no TypeScript, no build step) and has zero runtime dependencies: ignore the type-stripping material and never add packages to package.json. Diagnostic tools may run once with npx only. Tests: `npm test`; lint: `npm run lint`.
