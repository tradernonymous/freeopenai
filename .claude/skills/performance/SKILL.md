---
name: performance
description: "Web performance. Use when the web page feels slow, index.html or vendored assets grow, first load or streaming is sluggish, or before adding heavy scripts."
license: MIT
metadata:
...
> **freeopenai notes:** the web app is one large index.html plus vendored scripts served by server.js on Railway (no CDN/edge layer). Measure first (browser devtools or `npm run smoke`); label unmeasured findings as estimates.
