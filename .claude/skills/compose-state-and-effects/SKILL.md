...
description: "Compose state ownership and effects. Use when writing or reviewing remember/mutableState, view model state, LaunchedEffect, DisposableEffect, snackbars, focus or navigation state in android/."
---

> **freeopenai notes:** the app keeps screen state in an AndroidViewModel with Compose mutableState written only on the main thread; there is no local Android SDK, so verify through the Android GitHub Actions run.
...
7. Route frame-rate reads, cross-phase back-writing, and
   `@ReadOnlyComposable` contracts to Compose performance guidance.
