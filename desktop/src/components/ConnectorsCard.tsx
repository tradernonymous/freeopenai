...
export default function ConnectorsCard() {
  ...
  const refreshGithub = useCallback(() => {
    api.raw('/api/github/status')
      ...
      .catch((e: unknown) => {
        setAccounts([]);
        setGithubNote(((e as Error).message || String(e)).split('\n')[0]);
      });
  }, []);
  ...
  useEffect(() => {
    ...
    onConnectFinished(() => { refreshGithub(); pushToast('ok', 'GitHub connected.'); }).then((off) => { stop = off; });
    return () => { window.removeEventListener(tools.CHANGED_EVENT, onTools); stop(); };
  }, [refreshGithub]);

  const connectGithub = () => {
    const page = `${api.getServer().replace(/\/+$/, '')}/api/github/authorize`;
    if (hasShell()) {
    ...
    <section className="settings-section">
      ...
      <div className="settings-card">
            ...
            {accounts.map((account) => (
              <div key={account.login} className="local-row">
                <div className="local-row-main">
                  <span className="local-row-name"><span className="mono">{account.login}</span></span>
        ...
        <div className="local-status-row">
          ...
          <button onClick={refreshGithub}>Refresh</button>
        </div>
