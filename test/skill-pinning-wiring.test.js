...
test('a command is resolved before anything is sent, and never sent to a model', () => {
  const send = APP_JS.slice(APP_JS.indexOf('async function sendMessage()'));
...
test('each command does what it says on the tin', () => {
  assert.match(APP_JS, /parsed\.name === 'help'[\s\S]{0,80}renderCommandsHelp\(\)/);
  assert.match(APP_JS, /parsed\.name === 'skills'[\s\S]{0,90}renderSkillsCommandReply\(activeSkillNames, catalog\)/);
  assert.match(APP_JS, /parsed\.name === 'mode'[\s\S]{0,60}applyModeCommand\(parsed\.args\)/);
  assert.match(APP_JS, /parsed\.name === 'clear'[\s\S]{0,200}startNewConversation\(\)/);
  assert.match(APP_JS, /parsed\.name === 'skill'[\s\S]{0,60}applySkillCommand\(parsed\.args\)/);
  // `/ponytail` is the shorthand for `/skill ponytail`.
  assert.match(APP_JS, /parsed\.kind === 'skill'[\s\S]{0,60}pinSkillForChat\(parsed\.name\)/);
  // And `/skill off` has to be able to switch one off, or the cap is a trap.
  assert.match(APP_JS, /function applySkillCommand[\s\S]{0,200}\/\^off\\b/);
  assert.match(APP_JS, /setActiveSkillNames\(deactivateSkill\(activeSkillNames, which\)\)/);
});
...
test('the pinned set belongs to the chat: saved with it, loaded from it, absent in a new one', () => {
  ...
  const sync = APP_JS.slice(APP_JS.indexOf('function syncActiveSkillsFromConversation'), APP_JS.indexOf('function renderSkillBar'));
...
test('a pinned skill is consulted even in Chat mode, where nothing is auto-picked', () => {
  ...
  assert.match(APP_JS, /skillsForTurn\(\{[\s\S]{0,200}active: activeSkillNames,/);
  ...
  assert.ok(APP_JS.includes('if (activeSkills.length)'), 'active-skills block present');
  assert.ok(APP_JS.includes('filterToolsBySkills'), 'allowed-tools scoping is wired');
  assert.ok(APP_JS.includes('tools.push(USE_SKILL_TOOL)'), 'use_skill tool still offered');
  assert.ok(APP_JS.includes('logSkillsUsed(activeConversationId, activeSkills)'), 'skill application still logged');
  // A plain chat with nothing pinned does not drag the library in at all.
  const gate = APP_JS.slice(APP_JS.indexOf('if (activeSkillNames.length || learnedSkills'), APP_JS.indexOf('const stored = readPendingTurn()'));
...
test('a skill the model loads is pinned, so the next turn does not fetch it again', () => {
  const tool = APP_JS.slice(APP_JS.indexOf('async function runUseSkillTool'), APP_JS.indexOf('// Set when GitHub handed back'));
  assert.match(tool, /pinSkillQuietly\(name\)/, 'use_skill has to stick to the chat');
  const pin = APP_JS.slice(APP_JS.indexOf('function pinSkillQuietly'), APP_JS.indexOf('// --- The skill picker ---'));
...
test('the composer has buttons: a chip per pinned skill, and a picker in the panel', () => {
  ...
  const bar = APP_JS.slice(APP_JS.indexOf('function renderSkillBar'), APP_JS.indexOf('function removePinnedSkill'));
...
test('the skills panel and the picker tell one story about the library', () => {
  ...
  assert.match(APP_JS, /skillsStatusText = skillsCatalog\.length/);
  assert.match(APP_JS, /empty\.textContent = skillsStatusText/);
  assert.match(APP_JS, /pinned ones apply in every mode/);
...
test('the switch and the picker sit in one section, and the panel owns both', () => {
  ...
  const outsideClick = APP_JS.slice(APP_JS.indexOf("document.addEventListener('click', () => {"), APP_JS.indexOf("});", APP_JS.indexOf("document.addEventListener('click', () => {")));
...
test('offers are scored while typing, before the message they are for is sent', () => {
  ...
  assert.match(APP_JS, /chatInput\.addEventListener\('input', scheduleSkillSuggestion\)/);
  assert.match(APP_JS, /function scheduleSkillSuggestion\(\)[\s\S]{0,300}setTimeout\([\s\S]{0,120}updateSkillSuggestion\(\)/);
  assert.match(APP_JS, /function updateSkillSuggestion\(\)[\s\S]{0,400}suggestSkillFor\(chatInput\.value, skillsCatalog, skillUsage/);
  // Both halves of "do not offer this again": what is pinned, and what was refused.
  assert.match(APP_JS, /active: activeSkillNames,\s+dismissed: activeSkillDismissals,/);
...
test('accepting an offer pins it and teaches the app; dismissing it is remembered per chat', () => {
  ...
  assert.match(APP_JS, /function acceptSkillSuggestion\(\)[\s\S]{0,300}pinSkillForChat\(name\)/);
  // Only deliberate pins count as a habit: the model's own use_skill must not.
  assert.match(APP_JS, /function rememberSkillHabit\(name\)[\s\S]{0,300}recordSkillPin\(skillUsage, name, activeConversationId\)/);
  const pin = APP_JS.slice(APP_JS.indexOf('function pinSkillForChat'), APP_JS.indexOf('function pinSkillQuietly'));
  assert.match(pin, /rememberSkillHabit\(row\.name\)/);
  const quiet = APP_JS.slice(APP_JS.indexOf('function pinSkillQuietly'), APP_JS.indexOf('// --- The skill picker ---'));
  ...
  assert.match(APP_JS, /function dismissSkillSuggestion\(\)[\s\S]{0,500}activeSkillDismissals = \[\.\.\.activeSkillDismissals, name\]/);
  assert.match(APP_JS, /skillsDismissed: activeSkillDismissals,/);
...
test('the offer is visible with nothing pinned, and says why it is being made', () => {
  ...
  const bar = APP_JS.slice(APP_JS.indexOf('function renderSkillBar'), APP_JS.indexOf('function buildSkillSuggestionChip'));
  ...
  const chip = APP_JS.slice(APP_JS.indexOf('function buildSkillSuggestionChip'), APP_JS.indexOf('function removePinnedSkill'));
